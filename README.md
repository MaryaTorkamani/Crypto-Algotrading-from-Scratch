...
At the first step, we need to capture our data from different sources. There are several methods for doing so, but we are using yfinance library in python.Due to  some limitation in yahoo finance database regarding some tokens, We also aim to acquire our required market data through Binance API.
